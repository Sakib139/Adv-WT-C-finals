...
const Signin = ({onHandleSignIn}) => {
  ...
  const handleSubmit = (e) => {
    ...
    .then(response=>{
        ...
        console.log(data);
        if(data.type === 'user'){
            var user = {userid: data.email, access_token: data.token};
  ...
  return (
        <div style={{ width: '55%', marginTop: '10%', marginLeft: '20%', boxShadow: '4px 3px 8px 1px', padding: '40px' }}>
            <h3 className="mb-0" style={{ textAlign: 'center' }}>MedEx: User</h3> <br/>
